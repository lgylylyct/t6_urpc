...
def train(model, train_generator, optimizer, criterion, logger, config, epoch):
    ...
    scaler = torch.cuda.amp.GradScaler()    ## automatic mixing accuracy calculation
    ...
    for idx in range(num_iter):
        ...
        with torch.cuda.amp.autocast():
            ...
            loss = l_dc + l_sim   ##最重要的就是这个损失函数

        losses.update(loss.item(), config.TRAIN.BATCH_SIZE)   ##其实这个应该也没有什么用
        # do back-propagation
        optimizer.zero_grad()
