...
def conv3x3(in_planes, out_planes, kernel_size, stride=(1, 1), padding=(0, 0), dilation=(1, 1), bias=False,
            weight_std=False):
    "3x3 convolution with padding"
    if weight_std:
        return Conv2d_wd(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding,
                         dilation=dilation, bias=bias)
    else:
...
class Conv2dBlock(nn.Module):
    def __init__(self, in_channels, out_channels, norm_cfg, activation_cfg, kernel_size, stride=1, padding=0,
                 dilation=1, bias=False, weight_std=False):
        super(Conv2dBlock, self).__init__()
        self.conv = conv3x3(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding,
