...
from Untils.pos_embed import get_2d_sincos_pos_embed
import cv2
import numpy as np
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def patchify(self, imgs):
        """
        imgs: (N, 3, H, W)
        x: (N, L, patch_size**2 *3)   gain the target for cal the loss of the data
        change : x(N, L, patch_size**2)           因为我更改了对应的通道数目  所以最后得到的也会不一样
        """
        p = self.patch_embed.patch_size[0]
        assert imgs.shape[2] == imgs.shape[3] and imgs.shape[2] % p == 0

        h = w = imgs.shape[2] // p
        x = imgs.reshape(shape=(imgs.shape[0], 1, h, p, w, p))       ## 更改通道数目,3->1
        x = torch.einsum('nchpwq->nhwpqc', x)
        x = x.reshape(shape=(imgs.shape[0], h * w, p ** 2 * 1))      ##更改通道数目 3->1
        return x
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        x = x + self.pos_embed[:, 1:, :]   ##[2,196,384]

        x, mask, ids_restore = self.random_masking(x, mask_ratio)
        #[2,49,384][剩下的49是没有被遮盖的]  [2,196][0/1,按照比率遮盖]  [2,196][按照196进行编号]
        # append cls token
...
if __name__=='__main__':
     ...
     model_test = mae_vit_small_patch16_dec512d8b().to(torch.device('cuda'))
     print(summary(model_test, (3, 224, 224)))
