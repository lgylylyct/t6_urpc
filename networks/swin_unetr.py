...
class Decoder_SwinUNETR(nn.Module):
    def __init__(
        self, spatial_dims, feature_size, norm_name, in_channels, out_channels
    ) -> None:
        super().__init__()
...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        self.main_decoder = Decoder_SwinUNETR(
            spatial_dims, feature_size, norm_name, in_channels, out_channels
        )
        self.aux_decoder1 = Decoder_SwinUNETR(
            spatial_dims, feature_size, norm_name, in_channels, out_channels
        )
        self.aux_decoder2 = Decoder_SwinUNETR(
            spatial_dims, feature_size, norm_name, in_channels, out_channels
        )
        self.aux_decoder3 = Decoder_SwinUNETR(
            spatial_dims, feature_size, norm_name, in_channels, out_channels
        )
    ...
    def forward(self, x_in):
        ...
        main_seg = self.main_decoder(x_in, hidden_states_out)
        aux1_feature = [FeatureNoise()(i) for i in hidden_states_out]
        aux_seg1 = self.aux_decoder1(x_in, aux1_feature)
        aux2_feature = [Dropout(i) for i in hidden_states_out]
        aux_seg2 = self.aux_decoder2(x_in, aux2_feature)
        aux3_feature = [FeatureDropout(i) for i in hidden_states_out]
        aux_seg3 = self.aux_decoder3(x_in, aux3_feature)
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        if mask is not None:
            nw = mask.shape[0]
            attn = attn.view(b // nw, nw, self.num_heads, n, n) + mask.unsqueeze(1).unsqueeze(
                0
            )
